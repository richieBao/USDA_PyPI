...
def get_scheduler(optimizer, opt):
    ...
    if opt.train.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.train.saveload.epoch_count - opt.train.n_epochs) / float(opt.train.n_epochs_decay + 1)
            return lr_l
        ...
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.train.n_epochs, eta_min=0)
    else:
