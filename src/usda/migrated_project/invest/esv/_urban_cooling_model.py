# -*- coding: utf-8 -*-
"""
Created on Fri Jun  9 20:46:53 2023
...
if __package__:
    ...
    from ..configs import cfg_urban_cooling
else:
    import _utils as utils
    from configs import cfg_urban_cooling

TARGET_NODATA=cfg_urban_cooling.TARGET_NODATA
...
class Urban_cooling:
    ...
    def __init__(self,args):
        ...
        cc_park_task = task_graph.add_task(
            func=self.convolve_2d_by_exponential,
            args=(
        ...
        if args['cc_method'] == 'factors':
            ...
            eti_task = task_graph.add_task(
                ...
                args=(
                    ...
                    self.calc_eti_op, eti_raster_path, gdal.GDT_Float32, TARGET_NODATA),
            ...
            cc_task = task_graph.add_task(
                ...
                args=([(task_path_prop_map['shade'][1], 1),
                       ...
                       (cc_weight_eti, 'raw')],
                      self.calc_cc_op_factors, cc_raster_path,
            ...
            cc_task = task_graph.add_task(
                ...
                args=([(task_path_prop_map['building_intensity'][1], 1)],
                      self.calc_cc_op_intensity, cc_raster_path,
                      gdal.GDT_Float32, TARGET_NODATA),
                target_path_list=[cc_raster_path],
                dependent_task_list=[
        ...
        hm_task = task_graph.add_task(
            ...
            ], self.hm_op, hm_raster_path, gdal.GDT_Float32, TARGET_NODATA),
        ...
        t_air_nomix_task = task_graph.add_task(
            ...
            args=([(t_ref_raw, 'raw'),
                   (hm_raster_path, 1),
                   (uhi_max_raw, 'raw')],
                  self.calc_t_air_nomix_op, t_air_nomix_raster_path, gdal.GDT_Float32,
                  TARGET_NODATA),
        ...
        if bool(args['do_energy_valuation']):
            ...
            _ = task_graph.add_task(
                func=self.calculate_energy_savings,
                args=(
        ...
        _ = task_graph.add_task(
            func=self.calculate_uhi_result_vector,
            args=(
                intermediate_aoi_vector_path,
    ...
    def calculate_uhi_result_vector(self,
    ...
    def calculate_energy_savings(self,
        ...
        for target_index, target_feature in enumerate(target_building_layer):
            last_time = self._invoke_timed_callback(
    ...
    def calc_t_air_nomix_op(self,t_ref_val, hm_array, uhi_max):
    ...
    def calc_cc_op_intensity(self,intensity_array):
    ...
    def flat_disk_kernel(self,max_distance, kernel_filepath):
    ...
    def hm_op(self,cc_array, green_area_sum, cc_park_array, green_area_threshold):
        """Calculate HM.
    ...
    def _invoke_timed_callback(self,
    ...
    def convolve_2d_by_exponential(self,
